...
@SpringApplicationConfiguration(classes = SampleNutaxiSsoApplication.class)
@WebAppConfiguration
@IntegrationTest({ "debug", "server.port:0" })
public class SampleNutaxiSsoApplicationTests {
	...
	public void userEndpointProtected() {
		ResponseEntity<String> response = template.getForEntity("http://localhost:" + port + "/dashboard/user",
				String.class);
	...
	public void loginRedirects() {
		ResponseEntity<String> response = template.getForEntity("http://localhost:" + port + "/dashboard/login",
				String.class);
		assertEquals(HttpStatus.FOUND, response.getStatusCode());
		String location = response.getHeaders().getFirst("Location");
